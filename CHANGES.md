# 변경 사항 — 다운로드 효율 개선 및 안정성 보강

## 후속 수정 — 직접 URL 프록시 제거

빠름 모드에서 YouTube CDN 직접 URL을 아이폰에 프록시하던 경로를 제거했다. 영상별 HTTP
헤더·HLS·PO token 조건에 따라 CDN URL이 `403`을 반환하거나 MP4 대신 `.m3u8` 매니페스트가
전달될 수 있었기 때문이다.

- 빠름/고화질 모두 서버의 요청별 임시 디렉토리에 `yt-dlp`로 파일을 저장한다.
- 두 모드 모두 `/result/{job_id}`의 `FileResponse`로 동일 출처 첨부 파일을 제공한다.
- 고화질 포맷은 HLS MP4를 우선 선택하고 DASH 포맷으로 fallback한다.
- Node.js EJS 런타임을 `--js-runtimes node`로 활성화한다.
